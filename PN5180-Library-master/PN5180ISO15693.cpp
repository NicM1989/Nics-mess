...
// *****Changes made by Nic Morgan 1/09/2019*******
// - Added Function for UID-addressed command to get 1 tag to respond if it is called
// ************************************************
#define DEBUG 1
 ...
 *
 */
 //******************* Nic's Function get addressed UID **************************************
 
 ISO15693ErrorCode PN5180ISO15693::sendQuiet() {
  //                     
   uint8_t getuid[8]; // Create temporary array for reading a tag UID from arduino EEPROM
	 struct eepromUIDread read1;
	 EEPROM.get(16, read1);	// Read the next UID in the library
	 for(int i=0; i<8; i++){
	 getuid[i] = read1.tempUIDread [i];
     }				//	Flags,  CMD,   UID
  uint8_t addpacket[] = { 0x22, 0x02, getuid[0],getuid[1],getuid[2],getuid[3],getuid[4],getuid[5],getuid[6],getuid[7]};
  //						||     \- command VICC to 'QUIET' state 
  //                        |\- inventory flag LOW + high data rate
  //                        \-- addressed for UID, no AFI field present
  //
  PN5180DEBUG(F("Sending UID "));
 for (int k = 0; k < 8; k++) {
      Serial.print(getuid[7 - k], HEX); // LSB is first   //print RFID tag 8-character hex ID to serial monitor
      if (k < 2) Serial.print(":");
    }
  Serial.print(F("Quiet "));
  delay(2000);
}
 //*********************** End Nic's Addressed UID function ***********************************
...
ISO15693ErrorCode PN5180ISO15693::issueISO15693Command(uint8_t *cmd, uint8_t cmdLen, uint8_t **resultPtr) {
  ...
  if (0 == (getIRQStatus() & RX_SOF_DET_IRQ_STAT)) {  //If SOF not detected in IRQ flag register then no card present
    return EC_NO_CARD;
  }
